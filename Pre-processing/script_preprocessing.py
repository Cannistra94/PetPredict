...
#PRINT PIXEL AND IMAGE INFORMATION
...
def n4_bias_correction(input_image_filename, output_image_filename, mask_image_filename=None, figures_directory=None):
    ...
    if figures_directory:
        original_figure_filename = os.path.join(figures_directory, os.path.basename(input_image_filename).replace('.nii.gz', '_original.png'))
    ...
    if figures_directory:
        corrected_figure_filename = os.path.join(figures_directory, os.path.basename(output_image_filename).replace('.nii.gz', '_corrected.png'))
...
for i in images.iloc[:, 0]:
    input_filename = f"nifti_images\\{i}.nii.gz"
    output_filename = f"{output_dir}/{i}.nii.gz"
    n4_bias_correction(input_filename, output_filename, figures_directory=figures_dir)

output_txt_path = "output.txt"
with open(output_txt_path, "a") as file:
    file.write("Images corrected for field inhomogeneity using N4BiasFieldCorrection algorithm (sitk library) succesfully\n")
...
def process_images(csv_file,output_folder):
    images = pd.read_csv(csv_file, header=0)
    os.makedirs(output_folder, exist_ok=True)
    for i in range(len(images)):
        image_path = os.path.join("n4_corrected", images.iloc[i, 0] + ".nii.gz")
        
        output_path = os.path.join(output_folder, images.iloc[i, 0] + "_normalized" + ".nii.gz")
        ...
        # Normalize images
        norm_image = normalize_image(image_array)
        

        
        # Convert back to SimpleITK images and save
        norm_matched_sitk = sitk.GetImageFromArray(norm_image)
        norm_matched_sitk.CopyInformation(image)  # Preserve metadata
        sitk.WriteImage(norm_matched_sitk, output_path)
...
# Example usage
process_images('patient_data.csv',  'normalized_images')
